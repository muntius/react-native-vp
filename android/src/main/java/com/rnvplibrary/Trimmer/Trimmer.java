...
public class Trimmer {
  ...
  private static final String SI_FILE_NAME = "ffprobe";
  ...
  private static class FfmpegCmdAsyncTask extends AsyncTask<FfmpegCmdAsyncTaskParams, Void, Void> {
    ...
    protected Void doInBackground(FfmpegCmdAsyncTaskParams... params) {
      ...
      try {
        ...
        Log.d(LOG_TAG, "cmd:"+cmd.toString());
        ...
        if ( errorCode != 0 ) {
          ...
          Log.d(LOG_TAG, "ffmpeg error: " + errorMessageFromCmd);
        }
      } catch (Exception e) {
    ...
    protected Void doInBackground(LoadFfmpegAsyncTaskParams... params) {
      ...
      try {
        ...
        if ( !(ffmpegFile.exists() && getSha1FromFile(ffmpegFile).equalsIgnoreCase(FFMPEG_SHA1)) ) {
          ...
          int n;
          InputStream ffmpegInAssets;
          InputStream ffmpegInAssets2 = null;
...
//    int cropHeight = (int)( options.getDouble("cropHeight") );
    ...
    int videoBitrate = (int)( options.getInt("bitrate") );
    ...
    cmd.add("-vf");
    if ( width != 0 && height != 0 ) {
      cmd.add("crop=" + Integer.toString(cropWidth) + ":" + Integer.toString(cropHeight) + ":" + Integer.toString(cropOffsetX) + ":" + Integer.toString(cropOffsetY)+ ","+ "scale=" + Integer.toString(width/(int) 2.25) + ":" + Integer.toString(height/(int) 2.25));
    } else {
